...
function Field({ label, name, value }: { label: string; name: string; value: string }) {
  return (
    <div className="flex flex-col gap-[5px]">
      <span className="font-rethink text-[12px] font-medium text-[#909090] lowercase tracking-wider">{label}</span>
      <Input
        title={label}
        name={name}
        placeholder={label}
...
export default function EmployeeDetail({
  ...
  const schemaById = new Map(employeeSchema.map((field) => [field.id, field.label]))
  const schemaByKey = new Map(employeeSchema.map((field) => [field.key, field.label]))
  const roleLabel = schemaByKey.get("role") ?? "Role"
  const statusLabel = schemaByKey.get("status") ?? "Status"
  const nameLabel = schemaByKey.get("name") ?? "Employee Name"
  const emailLabel = schemaByKey.get("email") ?? "Employee Email"
          ...
          <form onSubmit={(e) => handleSubmit(e, id)}>
            <div className="space-y-4 p-4">
              ...
              <div className="grid grid-cols-2 gap-x-4">
                <Field label={roleLabel} name="role" value={employee.role ?? ""} />
                <Field label={statusLabel} name="status" value={employee.status ?? ""} />
              </div>
            </div>
